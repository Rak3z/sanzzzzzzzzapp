<?php
session_start();
if(!empty($_POST) && (!empty($_POST['id_prod'])|| !empty($_POST['borrar']))){
    ...
    if(isset($_POST['unid_disp'])){
        if(empty($_POST['unid_disp'])){
            ...
            $var = $_POST['unid_disp'];
            $sql = "SELECT * FROM productos WHERE id_producto = '$uid'";
            foreach($pdo->query($sql) as $fila){//query llega
                $cant_min = $fila['unidades_vend'];
            }
            if ($var> $cant_min){
    ...
    if(isset($_POST['borrar'])){
        if(empty($_POST['borrar'])){
            ...
            $var = $_POST['borrar'];
            echo $var;
            $sql = "DELETE FROM productos WHERE id_producto = '$var'";
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
        }
    }
    ...
    <body>
        <section id = "container">
            ...
            <form action = "" method= "post">
                ...
                <input type = "text" name ="borrar" placeholder = "id del producto a borrar">   
                <div class ="alert" <?php echo isset($alert)? $alert : ""; ?>></div>
