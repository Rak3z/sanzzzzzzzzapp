...
if(!empty($_POST)){
    ...
    /*if(isset($_POST['rol'])){
        if( empty($_POST['rol']) || $_POST['rol'] != $uid){
            $alert = "ingrese rol valida";
        }else{
            $var = $_POST['fecha_n'];
            $sql = "UPDATE usuario SET fecha_nacimiento = NULL WHERE rol = '$uid'";
            $sql = "UPDATE usuario SET nombre = NULL WHERE rol = '$uid'";        
            $sql = "UPDATE usuario SET correo = NULL WHERE rol = '$uid'";
            $stmt = $pdo->prepare($sql);
            $stmt->execute();
        }
    }
